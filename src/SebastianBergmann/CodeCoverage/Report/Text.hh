...
class Text {
  ...
  public function process(CodeCoverage $coverage, $showColors = false) {
    ...
    $paddingData = array();
    $paddingData[] = strlen($classes);
    $paddingData[] = strlen($methods);
    $paddingData[] = strlen($lines);

    $padding = max($paddingData);
    ...
    foreach ($report->getFiles() as $item) {
      ...
      foreach ($classes as $className => $class) {
        ...
        $c_namespace = $class->package->get('namespace');
        $c_fullPackage = $class->package->get('fullPackage');

        if ($c_namespace !== null) {
          $namespace = '\\'.$c_namespace.'::';
        } else if ($c_fullPackage !== null) {
          $namespace = '@'.$c_fullPackage.'::';
        } else {
          $namespace = '';
        }
    ...
    foreach ($classCoverage as $fullQualifiedPath => $classInfo) {
      ...
      if ($classInfo['methodsCovered'] === $classInfo['methodCount'] &&
          $classInfo['statementsCovered'] === $classInfo['statementCount']) {
        $shouldDisplay = false;
      }
      ...
      if (($classInfo['statementsCovered'] != 0 ||
          ...
          $shouldDisplay === true) {
        if ($showColors) {
          $methodColor = $this->getCoverageColor(
            $classInfo['methodsCovered'],
            $classInfo['methodCount'],
          );
          $linesColor = $this->getCoverageColor(
            $classInfo['statementsCovered'],
            $classInfo['statementCount'],
          );
  ...
  protected function getCoverageColor(
    $numberOfCoveredElements,
    $totalNumberOfElements,
  ) {
    $coverage =
      Util::percent($numberOfCoveredElements, $totalNumberOfElements);
  ...
  protected function printCoverageCounts(
    int $numberOfCoveredElements,
    ...
    return
      ...
      ' ('.
      sprintf('%3s', $numberOfCoveredElements).
      '/'.
      sprintf('%3s', $totalNumberOfElements).
      ')';
  }
