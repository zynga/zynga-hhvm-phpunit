...
class PHPUnit_Util_Configuration {
  ...
  protected function __construct($filename) {
    ...
    $this->document =
      PHPUnit_Util_XML::loadFile($filename, false, true, true);
    ...
    if ($tmp->length == 1) {
      if ($tmp->item(0)->hasAttribute('addUncoveredFilesFromWhitelist')) {
        $addUncoveredFilesFromWhitelist = $this->getBoolean(
          (string) $tmp->item(0)
            ->getAttribute('addUncoveredFilesFromWhitelist'),
          true,
        );
      }
      ...
      if ($tmp->item(0)->hasAttribute('processUncoveredFilesFromWhitelist')) {
        $processUncoveredFilesFromWhitelist = $this->getBoolean(
          (string) $tmp->item(0)
            ->getAttribute('processUncoveredFilesFromWhitelist'),
          false,
        );
      }
    }
      ...
      'whitelist' =>
        [
          ...
          'include' => [
            ...
            'file' => $this->readFilterFiles('filter/whitelist/file'),
          ],
          'exclude' => [
            'directory' => $this->readFilterDirectories(
  ...
  public function getListenerConfiguration() {
    ...
    foreach ($this->xpath->query('listeners/listener') as $listener) {
          ...
          foreach ($node->childNodes as $argument) {
            if ($argument instanceof DOMElement) {
              if ($argument->tagName == 'file' ||
                ...
                $arguments[] =
                  $this->toAbsolutePath((string) $argument->textContent);
              } else {
      ...
      $result[] =
        ['class' => $class, 'file' => $file, 'arguments' => $arguments];
    }
  ...
  public function getLoggingConfiguration() {
    ...
    foreach ($this->xpath->query('logging/log') as $log) {
      ...
      } else if ($type == 'coverage-crap4j') {
        if ($log->hasAttribute('threshold')) {
          $result['crap4jThreshold'] =
            $this->getInteger((string) $log->getAttribute('threshold'), 30);
        }
  ...
  public function handlePHPConfiguration() {
    ...
    foreach (['var', 'post', 'get', 'cookie', 'server', 'files', 'request'] as
             $array) {
  ...
  public function getPHPUnitConfiguration() {
    ...
    if ($root->hasAttribute('cacheTokens')) {
      $result['cacheTokens'] =
        $this->getBoolean((string) $root->getAttribute('cacheTokens'), false);
    }
    ...
    if ($root->hasAttribute('stderr')) {
      $result['stderr'] =
        $this->getBoolean((string) $root->getAttribute('stderr'), false);
    }
    ...
    if ($root->getAttribute('bootstrap')) {
      $result['bootstrap'] =
        $this->toAbsolutePath((string) $root->getAttribute('bootstrap'));
    }
    ...
    if ($root->hasAttribute('stopOnError')) {
      $result['stopOnError'] =
        $this->getBoolean((string) $root->getAttribute('stopOnError'), false);
    }
    ...
    if ($root->hasAttribute('stopOnRisky')) {
      $result['stopOnRisky'] =
        $this->getBoolean((string) $root->getAttribute('stopOnRisky'), false);
    }
    ...
    if ($root->hasAttribute('failOnRisky')) {
      $result['failOnRisky'] =
        $this->getBoolean((string) $root->getAttribute('failOnRisky'), false);
    }

    if ($root->hasAttribute('testSuiteLoaderClass')) {
      $result['testSuiteLoaderClass'] =
        (string) $root->getAttribute('testSuiteLoaderClass');
    }
    ...
    if ($root->hasAttribute('printerClass')) {
      $result['printerClass'] = (string) $root->getAttribute('printerClass');
    }

    if ($root->getAttribute('printerFile')) {
      $result['printerFile'] =
        $this->toAbsolutePath((string) $root->getAttribute('printerFile'));
    }
    ...
    } else if ($root->hasAttribute('checkForUnintentionallyCoveredCode')) {
      $result['strictCoverage'] = $this->getBoolean(
        (string) $root->getAttribute('checkForUnintentionallyCoveredCode'),
        false,
      );

      $result['deprecatedCheckForUnintentionallyCoveredCodeSettingUsed'] =
        true;
    }
    ...
    if ($root->hasAttribute('verbose')) {
      $result['verbose'] =
        $this->getBoolean((string) $root->getAttribute('verbose'), false);
    }
  ...
  protected function getTestSuite(
    ...
    if ($testSuiteNode->hasAttribute('name')) {
      $suite = new TestSuite((string) $testSuiteNode->getAttribute('name'));
    } else {
      $suite = new TestSuite();
    }
    ...
    $testSuiteFilter =
      $testSuiteFilter
        ? explode(self::TEST_SUITE_FILTER_SEPARATOR, $testSuiteFilter)
        : [];

    foreach ($testSuiteNode->getElementsByTagName('directory') as
             $directoryNode) {
      if (!empty($testSuiteFilter) &&
          !in_array(
      ...
      if ($directoryNode->hasAttribute('phpVersionOperator')) {
        $phpVersionOperator =
          (string) $directoryNode->getAttribute('phpVersionOperator');
      } else {
        $phpVersionOperator = '>=';
      }
      ...
      $v_files = Vector {};
      $v_files->addAll($files);

      $suite->addTestFiles($v_files);

    }

    foreach ($testSuiteNode->getElementsByTagName('file') as $fileNode) {
      if ($testSuiteFilter &&
          $fileNode->parentNode->getAttribute('name') != $testSuiteFilter) {
        continue;
      }
      ...
      $file =
        $fileIteratorFacade->getFilesAsArray($this->toAbsolutePath($file));
