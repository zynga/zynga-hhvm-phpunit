...
class PHPUnit_Util_Configuration
{
    ...
    protected function getTestSuite(DOMElement $testSuiteNode, $testSuiteFilter = null) {
      ...
      // JEO: The global test suite is stood up here there is some goofyness going on
      // with the TestSuite node in the xml parser, we should take a peek as I had
      // to add a null else here.
      $suite = null;

      if ($testSuiteNode->hasAttribute('name')) {

        $className =  (string) $testSuiteNode->getAttribute('name');

        $reflected = ReflectionClasses::getReflection( $className );

        if ( $reflected instanceof ReflectionClass ) {
          $suite = new PHPUnit_Framework_TestSuite($reflected);
        } else {
          $suite = new PHPUnit_Framework_TestSuite(null);
        }
