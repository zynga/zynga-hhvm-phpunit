...
class Parser
{
    ...
    public function parse($value, $flags = 0)
    {
      ...
      $mbEncoding = null;
            ...
            if (preg_match('#^\-((?P<leadspaces>\s+)(?P<value>.+?))?\s*$#u', $this->currentLine, $values)) {
                ...
                if ($mergeNode) {
                    ...
                    if (!$this->isNextLineIndented() && !$this->isNextLineUnIndentedCollection()) {
                        ...
                        if ( is_string($key) && ($allowOverwrite || ! array_key_exists($key, $data)) ) {
                            $data[$key] = null;
                        }
                    } else {
                        ...
                        // Spec: Keys MUST be unique; first one wins.
                        // But overwriting is allowed when a merge node is used in current block.
                        if ( is_string($key) && ($allowOverwrite || !isset($data[$key]))) {
                            $data[$key] = $value;
                        }
                    }
                } else {
        ...
        if (Yaml::PARSE_OBJECT_FOR_MAP & $flags && !is_object($data) && 'mapping' === $context && is_array($data)) {
            $object = new \stdClass();
